  ...
  extends TypeIdResolverBase
{
  ...
  protected String idFromClass(Class<?> paramClass)
  {
    ...
    Class localClass = _typeFactory.constructType(paramClass).getRawClass();
    String str = localClass.getName();
    synchronized (_typeToId)
    {
      paramClass = (String)_typeToId.get(str);
      Object localObject = paramClass;
      if (paramClass == null)
      {
        if (_config.isAnnotationProcessingEnabled())
        {
          paramClass = _config.introspectClassAnnotations(localClass);
          paramClass = _config.getAnnotationIntrospector().findTypeName(paramClass.getClassInfo());
        }
